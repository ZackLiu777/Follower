...
struct TrendChart: View {
    ...
    static func barWidth(for count: Int) -> CGFloat {
        if count <= 7 { 24 } else if count <= 20 { 18 } else if count <= 50 { 14 } else { 10 }
    }
    ...
    private var xScaleDomain: ClosedRange<Date> {
        ...
        case .week:
            ...
            guard let first = dates.min(), let last = dates.max(), first < last else {
                return referenceDate...calendar.date(byAdding: .day, value: 1, to: referenceDate)!
            }
            return first...calendar.date(byAdding: .day, value: 1, to: last)!
        case .month:
    ...
    private var chartContent: some View {
        ...
        .chartXAxis {
            ...
            case .week:
                AxisMarks(values: dataPoints.map(\.date)) { value in
                    AxisValueLabel { if let d = value.as(Date.self) { Text(d, format: .dateTime.weekday(.abbreviated)).font(.caption2) } }
                }
            case .month:
