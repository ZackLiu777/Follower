...
15. **批量写入性能**：`MetricRepository.upsertBatch` 用 DELETE+INSERT（按复合键 `(accountId, metricType, window, observedAt)` 先删后插——v0.06 起替换 INSERT OR REPLACE，见 note 24）；`SnapshotRepository.upsertBatch` 用 DELETE+INSERT（snapshot 表无唯一索引——历史重复行会使加唯一索引的迁移失败）。均去掉逐条前导 SELECT，同步耗时从秒级降到毫秒级
...
20. **评论端点同域修复**（评论无法显示根因）：旧实现评论走 `graph.facebook.com/v21.0`（Facebook 域），而 App 的 token 是 Instagram Login 签发的 IGAA token（`api.instagram.com` 交换）——跨域 token 必被 Meta 拒绝（OAuthException 190），评论永远无法显示且与权限配置无关；2024 年 Basic Display 停用后评论端点已并入 `graph.instagram.com`（与主数据同域同 token），现全部评论方法（fetch/reply/delete）改走 `baseURL`，删除 facebookBaseURL 与 v21.0 版本号；新 API **不返回 username 字段**（fields 只用 id,text,timestamp，UI 兜底「Instagram 用户」）；`CommentViewModel.friendlyError` 解析 Meta 错误码（190→重新连接引导 / 200→商务账号提示 / 10→Meta 平台权限引导），`metaErrorCode` 为 nonisolated 纯函数；注意：Meta 开发模式下评论接口返回空数组（非报错）——正式使用需把 App 切到 Live 模式；**2026-08-09 实测确认**：真实 token（IGAA，App 1645583809858319 签发）+ 测试者身份（qqiangge 已生效）+ 权限已配置，`/me`（MEDIA_CREATOR）与 `/me/media` 正常（帖子 comments_count=6），但 `/media/comments` 与 `/me/insights` 均返回 HTTP 200 + `data:[]`——**开发模式（App 未发布）下未过审权限的扩展端点对所有人返回空数组，测试者也不例外**；另发现面板权限列表缺 `instagram_business_manage_insights`（insights 返回空与此相关，提交审核前需补齐）——详见 `docs/specs/instagram-app-review-plan.md`
...
22. **下拉刷新 = 增量同步（刷新无反应修复）**：原 `.refreshable` 只调 `loadAccounts()`（读本地库），下拉刷新永远看不到最新点赞数（需手动点「立即同步」）；现改为 `DashboardViewModel.incrementalSync()` → `SyncEngine.incrementalSync()`——**60 秒节流**（`minSyncInterval`，依据 `eventRepo.latestObservedAt`）：60 秒内已同步则跳过网络请求，防耗尽 Instagram API 配额（200 次/小时/用户）；同步失败时不写 Event → 节流窗口不生效 → 用户可立即重试（天然失败重试语义）；「立即同步」按钮保留强同步 `sync()` 不变；账号列表仍由 `.task` 与 `accountCreated` 通知维护，不再依赖下拉刷新
23. **媒体包 PDF 导出（Premium: mediaKitExport，v0.04 实现，v0.05 三模板+趋势统计）**：`Follower/Services/MediaKit/` 四文件——`MediaKitData`（收集产物纯模型，含 `MediaKitTemplate` 枚举：professional / creative / minimal）/ `MediaKitDataProvider`（Repository 唯一入口；**数据不全不抛错**，空序列以 nil/占位表达，仅账号不存在 throw；v0.05 加 `trendSeries` 5 指标 × 30 天日窗口）/ `MediaKitPDFGenerator`（管线：`UIGraphicsPDFRenderer` 逐页绘制 → **PDFKit `PDFDocument` 组装与页数校验** → 写入 `Documents/MediaKits/`，后台线程安全；页面经 `MediaKitPageDrawing` 协议注入；**页码从各页面硬编码改为渲染层统一绘制**——模板化后页数自适应，封面 `showsPageNumber=false`）/ `MediaKitPages`（**三模板**：professional=完整 10 页浅色专业风（默认）、creative=完整 10 页+**Instagram 渐变封面**（紫→粉→橙）、minimal=封面+指标+结语固定 3 页；页面组合按模板再按数据可用性裁剪——无帖子跳过内容页、无序列跳过互动页、无趋势数据跳过趋势页；**趋势柱状页**（6 指标 × 7 周竖柱状图，`drawVBarChart` 手绘：网格线+圆角柱+首尾数值标注，与趋势页同风格，互动率独立 y 上限）/ **趋势统计页 ×2**（第 1 页：粉丝 12 周周窗口+互动率+均赞；第 2 页：均评+均分享+触达，均 30 天日窗口）/ **增长建议页**（决策引擎生成 Top 3：标题+原因+行动列表，与 Decisions 页同源——`MediaKitDataProvider.buildActionCards` 复用 `FeatureExtractor → ScoringEngine → CardGenerator` 纯函数流水线，snapshots 非空才生成，CardGenerator 兜底最少 4 张取前 3）；A4，浅色专业风固定不随主题，缺失显示 —）；排版修改指引在 `MediaKitPages.swift` 顶部；**入口：Dashboard Premium 网格 mediaKitExport 卡片**（`MediaKitDetailView` 做实：三模板 segmented+说明 → 生成 → ShareLink；`DashboardViewModel` 注入 mediaKitService 并持有模板/URL/生成中状态；**Settings 导出区入口已移除**，`SettingsViewModel` 不再依赖 mediaKitService）；L10n 新增 `mediakit.*` 44 key（en/zh-Hans/zh-Hant/ja）；测试 `MediaKitTests`（收集映射 / 空数据占位 / 账号缺失 throw / professional 全量 10 页 / minimal 固定 3 页 / creative 无帖子 9 页 / 空数据降级 3 页）
24. **趋势图表刷新重复修复（v0.06）**：根因——`MetricRepository.upsertBatch` 的 INSERT OR REPLACE 替换语义**依赖唯一索引 `idx_metric_account_type_window`**；若历史库索引缺失，REPLACE 退化为普通 INSERT，每次下拉刷新全量追加 730 天行 → 趋势图表「刷新后重复添加」（sqlite 实测：有索引 3→3 行幂等，无索引 3→6 行重复）。修复双保险：① **写入不再依赖索引**——`upsertBatch` 改为按复合键先 DELETE 再 INSERT（与 Snapshot 同模式，同量级耗时），任何环境下幂等且顺带清理同键残留；② **MigrationV5**（`v5_metric_dedup_index`）一次性修复存量库——先删同键重复行（每组保留 MIN(id) 一行），再补齐缺失的唯一索引（`db.indexes(on:)` 检测，均幂等可重跑）；测试 `RepositoryTests.testMetricUpsertBatchIsIdempotent`（3 次批量 upsert 仍 1 行）+ `testMigrationV5DeduplicatesAndRebuildsIndex`（DROP 索引 → 造 3 行脏数据 → 跑 V5 → 1 行 + 索引重建 + 同键 INSERT 抛约束错误）
25. **统计图表数据层整数化与四窗口真实值（v0.07，View 零改动）**：View 层（TrendChart/TrendsView/TrendDetailView）**一字未改**，仅数据层——① **整数语义**：`Metric.value: Double → Int`；`MigrationV7`（`v7_metric_integer_values`）换算存量库（engagementTrend 0~1 比率 → 万分比整数，其余计数类 ROUND 取整）；互动率一律**万分比整数**（`AggregationService.engagementBasis`，0.0543 → 543）；`TrendDataPoint.value` 保持 Double（图表层输入契约，改它必然牵连 View），新增 `init(date:value: Int)` 便捷初始化 → `TrendChart.weeklyDataPoints` 内 Metric→DataPoint 映射无需任何改动即编译；② **周期末值取代平均数**：周/月/年窗口指标不再 fSum/cnt 平均，改取**组内最后一次真实快照的 6 项值**（`periodEndMetrics`，输入升序遍历、后到覆盖）——每根柱都对应数据库真实存在的快照值；③ **四窗口定义**：日窗口 = 今天 0–24h 真实同步采样点（`generateHourlyData` 改读 profileSnapshot Event，一个事件一个点，无事件显示空态——删除伪造 baseVal/24 平线；VM 注入 `eventRepo`，ContentView 装配 +1 行）；月窗口 = **当月内日粒度真实值**（`chartData(.month)` 从日指标过滤当月）；年窗口 = **所选年份 12 根月柱**（`chartData(.year)` 从月指标过滤所选年）；④ **单位修复**：`FeatureExtractor` 互动率失真（旧 `/100` 把真实值兜底成 0.5%），改 `/10000`（万分比→比率）+ 兜底 3.5→350；⑤ 适配：TimeSeriesEngine / MediaKitDataProvider 仅类型转换不改行为；测试：新增周周期末值非平均（2/8→存 8）、万分比 543、月/年周期末值（1000/1200→存 1200）、日窗口真实采样（3 事件→3 点）、MigrationV7 存量换算（0.0543→543、8250.5→8251）；ServicesTests 原「每帖平均」断言对齐实际存储值（day 指标存快照总量——命名不一致为既有行为，未改动）；注意：周图无数据天柱高 0（不可见），但格线与标签仍显示——属 View 布局，按「View 零改动」约束本轮不改
26. **日窗口重复数据修复（v0.08，源头抑制）**：根因——每次同步（刷新按钮 `sync()` 无节流 / 下拉 `incrementalSync` 60 秒节流 / 登录后）都经 `IngestionService.ingest` 追加 1 条 profileSnapshot Event（`observedAt = Date()`），日窗口（v0.07 起）把今天全部事件各画一个点——**数据未变化时重复刷新 → 图表出现「相同数据、时间不同」的重复点**（周/月/年窗口无此问题：Metric upsert 幂等 + 唯一索引）。修复：**写入前抑制**——`ingest` 先取账号最近一条 profileSnapshot（`eventRepo.fetch(eventType:limit:1)` 按 observedAt 倒序），解码对比**业务字段**（username/displayName/followers/following/media/likes/comments/shares/views/engagementRate，`IngestionService.isSameProfile` 纯函数；**排除 fetchedAt**——每次同步必然不同的时间元数据），完全一致则跳过 profile 事件写入（trend followerChange 照常写，无重复问题）；任何一项业务字段变化照常写入。**已有数据一个不动**（不合并、不删除——用户明确「不能改变数据原有的情况」）；不做历史清理迁移。无 token 兜底分支（全 0 profile）同受抑制。测试：`testIngestSkipsUnchangedProfile`（同值两次 ingest → 1 条事件，第二次 eventsCreated == 0）/ `testIngestWritesChangedProfile`（值变 → 2 条）/ `testIsSameProfileIgnoresFetchedAt`（纯函数：fetchedAt 不影响判同，业务字段变 → 不同）
27. **日窗口按指标独立显示变化（v0.09，View 零改动）**：真机复现——粉丝 2，加评论后本地刷新：事件级抑制（v0.08）按「整个资料」判断，评论变 → 写入新事件（粉丝仍 2）→ 粉丝曲线多一根同值柱（重复堆叠）、详情页总计 2+2=4（翻倍）。修复两处（均在 TrendsViewModel，View 零改动）：① **按指标过滤同值点**——`generateHourlyData` 生成每指标曲线时经 `changedPoints` 只保留「首个点 + 该指标值变化的点」（同值点跳过、保留首次时间）；事件表一个字节不动，所有真实观测完整保留——粉丝 2→2→5 显示 [2,5]，评论 5→6→6 显示 [5,6]，互不干扰（「仅仅对粉丝的变化进行改变」）；② **总计 = 窗口最新真实值**——`totalValue` 从求和改为 `chartData.last`（日窗口粉丝采样 2→5 显示 5 而非 7；周/月/年柱为绝对值，求和同样无意义，统一窗口末值）。测试：`testDayFiltersUnchangedPointsPerMetric`（粉丝 [2,5] / 评论 [5,6]）/ `testChangedPointsPureFunction`（空/全同值 1 点/折返全保留/同值序列合并）/ `testTotalValueReturnsLatestValue`（5 非 7）；既有 `testDayUsesRealSyncSamples`（三值全不同）不受影响
28. **日窗口小时桶合并（v0.10，View 零改动）**：真机复现——评论变化频繁，1 小时内多次加评论/刷新（18:05 评论 6 → 18:40 评论 7），两个点都在同一小时桶 → 柱状图 x 轴为小时刻度（`TrendChart` `unit: .hour`）→ 同小时多根柱画在同一柱位**重叠显示**。v0.09 同值过滤只能滤「值没变」的点，滤不了「同小时值变多次」。修复：`generateHourlyData` 点序列先经新增纯函数 `hourlyBuckets`——按小时分组、每桶保留**最后一次真实观测**（后到覆盖），输出按时间升序——再叠加 `changedPoints` 同值过滤（下一小时值未变 → 不新增柱）。效果：每小时最多一根柱，每根柱都是真实观测值；事件表一个字节不动、View 零改动。测试：`testHourlyBucketsPureFunction`（同小时 6→7 取 7 / 跨小时全保留 / 空）/ `testDaySameHourCommentChangesCollapse`（18 点桶 7、19 点桶 8 → [7,8] 无重叠）；既有测试事件均在不同小时（9h/12h/18h）不受影响
29. **互动率图表删除（v0.11，用户决策）**：互动率（`engagementRate = avgLikes / followers`，只算点赞）对单账号无对标意义、口径脆（公式无上限，小粉丝账号 avgLikes ≥ followers 时失真）、万分比存储（543 = 0.0543）在 UI 显示不直观——用户确认删除图表。删除范围（仅显示层）：① **趋势页**——`TrendsViewModel.visibleMetricTypes` 移除 `.engagementTrend`（6 → 5 指标，趋势页图表卡片自动消失，TrendsView/TrendChart **零改动**）；② **Dashboard**——互动率卡片（`engagementCard`）整体删除，顺带消除卡片上「0~1 比率未 ×100」的显示 bug（0.0543 曾显示 "0.1%"）；`KeyMetricsSection` 相应去掉 `engagementDelta` 参数、`deltaText` 删除百分比分支；`DashboardViewModel.engagementDelta` 属性与计算一并移除。**数据生成全部保留**：`AggregationService` 照常写 engagementTrend Metric（万分比整数）、`FeatureExtractor` 决策引擎特征、`MediaKitDataProvider` PDF 趋势页、Premium 互动质量评分预留均不受影响（`MetricType.engagementTrend` 枚举值保留）。测试：`testVisibleMetricTypesCount` 6→5、`testVisibleMetricTypesContainsCore` 移除 `.engagementTrend` 断言、`testDayUsesRealSyncSamples` 移除互动率万分比断言（数据层存储由 `ServicesTests.testEngagementTrendStoredAsBasisPoints` 继续覆盖）
...
32. **日线空态修复（v0.15-alpha，View 零改动）**：用户报告趋势页日窗口显示 "No trend data yet" 而周/月/年正常。根因——**v0.08 写入层抑制与 v0.07 日窗口数据源冲突**：日窗口 = 今天 0–24h 的 profileSnapshot 事件采样点（v0.07）；v0.08 在 profile 业务字段与最近一次观测完全一致时**跳过 profileSnapshot 事件写入**（只写 followerChange）→ 数据未变化的同步后今天无 profileSnapshot 事件 → 日线空态；而周/月/年读 Metric，`buildSnapshots` 从 followerChange 事件同样构建快照 → 正常。修复（数据层，View 零改动）：**删除 v0.08 写入层抑制**——`ingest` 无条件写 profileSnapshot 事件（每次同步都是一次真实观测，append-only 语义）；同值重复点已由 v0.09 `changedPoints`（同值过滤）+ v0.10 `hourlyBuckets`（小时桶）在显示层彻底解决，删除抑制不回归。删除 `isUnchanged` / `isSameProfile`；测试 `testIngestSkipsUnchangedProfile` 反转为 `testIngestWritesEverySyncAsObservation`（同值两次 → 2 条事件）、`testIsSameProfileIgnoresFetchedAt` 删除（同值过滤语义由显示层测试 `testDayFiltersUnchangedPointsPerMetric` 明确覆盖）。集成测试 `TestAccountSyncIntegrationTests` 不受影响。**第二层（展示层刷新联动）**：用户复测仍空——根因是 TabView 中 `TrendsViewModel` 存活（`ContentView` 以 `@State` 持有），仅靠 View `.task` 首次加载与账号切换刷新，**Dashboard 同步完成后 Trends 不重新加载** → 日窗口停留在同步前的空态。修复：新增全局通知 `Notification.Name.syncCompleted`——`DashboardViewModel.sync()/incrementalSync()` 成功后广播，`TrendsViewModel` init 监听并 `reloadAfterSync()`（重新 `loadTrends`），deinit 移除监听。测试 `testTrendsReloadsAfterSyncNotification`（Mock 全链路：同步前日窗口空 → sync → 广播 → VM 自动重载 → 日窗口有采样点）。
