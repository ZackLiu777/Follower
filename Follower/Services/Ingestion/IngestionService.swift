...
final class IngestionService: IngestionServiceProtocol {
    ...
    func ingest(
        ...
        // 1. Profile Snapshot → Event（每次同步都是一次真实观测，无条件写入）
        // v0.15：删除 v0.08 的「资料未变化跳过写入」抑制——它导致日窗口
        // （读 profileSnapshot 事件采样）在数据未变化的同步后显示空态；
        // 同值重复点已由显示层解决（v0.09 changedPoints 同值过滤 + v0.10
        // hourlyBuckets 小时桶），写入层不再需要抑制。
